...
private:
    void escape(const std::string& s, bool single_quotes_mode, bool ensure_ascii);

    void dumpKey(const std::string& s, bool ensure_ascii);
    ...
    void dumpString(const StringTag& tag, bool ensure_ascii);

    // TODO: IEEE754标准判断与处理、更高效安全的输出算法
    template<typename FloatType, std::enable_if_t<
        std::is_floating_point_v<FloatType>, int> = 0>
    void dumpFloat(FloatType x)
    {
        if (!std::isfinite(x)) {
            m_oa->writeChars("0.0", 4);
            return;
        }
